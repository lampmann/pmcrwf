...
The character creation wizard fills in **Speed** from the race you pick (walking speed, even for a race whose data lists more than one movement type) so a freshly-created character's Movement pool starts correct rather than reading 0/0 until you set Speed by hand. If the Race Library has nothing for that race — most often because the sheet was opened straight from disk rather than through a server, so the automatic `data/races.json` load never ran (see [Where game data comes from](#where-game-data-comes-from)) — it falls back to 30 ft, the walking speed of most PHB races, rather than leaving the field blank; it's an ordinary editable field either way.
