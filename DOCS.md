...
- **Engine gaps that block whole categories of feature** (each would unlock a batch of currently-`⚠ not automated` entries):
  ...
  - No `speed` target — nothing in `derived.js` reads `effFlat("speed")`, so writing to it would silently do nothing. (`ac` *is* a real target now — `checkBonus("ac")` reads `effFlat`/`effDice` the same as init/saves/skills; a flat bonus works, though a *replacement* base-AC formula like an alternate Unarmored Defense still doesn't, since there's no "override the formula" op.)
