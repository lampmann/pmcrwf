...
- Coverage today (404 entries): a handful of hand-written entries proving the schema (`effects/db/handwritten.js`), a full pass over the 2014 feats (`effects/db/feats.js`), and a partial pass over class/subclass features (`effects/db/classes-batch-*.js`) — **artificer, barbarian, cleric, fighter, monk, and the sidekick classes are converted**; druid is partly done; bard, paladin, ranger, rogue, sorcerer, warlock, and wizard aren't started. All of it is generated by an LLM conversion pass over your `data/` folder and merged only after passing `effects/tools/validate-db.js` — see `effects/tools/conversion-guide.md` for the schema/process used. Nothing in it talks to your character or your `data/` folder without you explicitly running it.
...
- **[Feature effects](#feature-effects-automatic-mechanics)** cover all 2014 feats (`effects/db/feats.js`) and six of the classes plus the sidekicks (`effects/db/classes-batch-*.js`), against only those targets that already exist on the sheet (initiative, saves, skills, passive perception, max HP, spell DC/attack, proficiency bonus, ability scores). Effects that need a weapons/attacks table (Sharpshooter, GWM, and similar) are recognized and serialized but stay disabled until that module exists. **Still to convert:** bard, paladin, ranger, rogue, sorcerer, warlock, wizard, the rest of druid, and every race — they fall back to `uses`-only pip trackers in the meantime.
