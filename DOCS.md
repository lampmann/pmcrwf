...
**Feature effects**
- Still to convert: the remaining unconverted features inside cleric, druid, fighter and monk, which fall back to `uses`-only pip trackers in the meantime. Everything else — all 2014 feats, all races/subraces, all twelve classes plus the sidekicks — is done (`effects/db/*.js`).
