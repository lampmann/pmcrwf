...
- **Conditions & Status** — **Death saves** (3 success / 3 failure boxes, plus a **Roll Death Save** button that rolls 1d20 to the dice log and auto-marks a box: 10+ success, &lt;10 failure, nat 20 clears saves and sets HP to 1, nat 1 marks two failures); **Exhaustion** level 0–6 with the cumulative 2014 effects shown; and toggles for the 14 **conditions** (hover any for a short effect reminder). Trackers only — they don't yet auto-apply their mechanical effects (disadvantage, halved HP, etc.) to the sheet's math.
