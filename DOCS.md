...
- The character creator reports rather than resolves an equipment line the data states as a filter ("any martial weapon"); names rather than grants magic items from the DMG p38 table (there's no random magic-item table on the sheet); shows but doesn't pick a background's *choose* proficiencies; and neither it nor Level Up walks you through an Ability Score Improvement — the ASI is listed as a gained feature and the scores are left to you.
...
- **Resistances, immunities and vulnerabilities** are displayed when a *feature* grants one, but there's no way to add one by hand and nothing applies them to incoming damage — the sheet doesn't model damage taken.
- The Combat round tracker has no legendary/lair actions, no initiative order for the whole table, and no link to limited-use features — spending a Bonus Action on Second Wind doesn't tick its pip.
- Conditions, exhaustion and death saves are tracked but not *applied*: ticking Poisoned doesn't impose its disadvantage.
