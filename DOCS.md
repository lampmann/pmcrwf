...
**+ New character** opens a five-step wizard that follows PHB'14 chapter 1 ("Step-by-Step Characters", p11–15) in the book's own order, because that's the order the choices depend on each other in — your race sets ability increases you can't apply until scores exist, and your class sets the hit die you can't compute HP from until you know CON:

1. **Choose a Race** (p11) — race and subrace, typed against your imported race list. The step shows the ability increases the choice grants (including "and two others of your choice" races like the Half-Elf, which give you the pickers).
2. **Choose a Class** (p11) — class, subclass, and **level, defaulting to 1** but editable up to 20, so you can start a character mid-campaign without levelling them up one dialog at a time.
3. **Determine Ability Scores** (p12–13) — **standard array** (15/14/13/12/10/8), **point buy** (the PHB's cost table and 27-point budget, with the remaining budget shown live and the step blocked while you're over it), **roll 4d6 drop lowest** (logged to the Event Log, like every other roll on the sheet), or **enter manually**. Racial increases are applied on top and shown as a running final score.
4. **Describe Your Character** (p13) — name and background.
5. **Choose Equipment** (p14) — your class's starting gold, and a pointer to the Equipment Library.
...
*Two steps are deliberately thin.* Step 4 collects name and background only — personality traits, ideals, bonds, flaws and backstory are on this document's **Not planned** list, so the wizard doesn't ask for fields the sheet has nowhere to put. Step 5 gives starting gold rather than handing out class/background equipment packs: 5e.tools stores those as `startingEquipment` reference strings that need their own resolver, and half-populating an inventory the sheet can't fully model yet would be worse than pointing at the library.

### Levelling up
**Level Up** (Features module) opens the same kind of dialog for an existing character. Pick which class gains the level — or **multiclass into a new one at level 1** — and the dialog previews the features you'll gain at that level, read from the same data the Features panel renders from, so what it promises is exactly what shows up.

For hit points you either **take the fixed average** or **roll the Hit Die** (PHB p15). Taking the average writes nothing: Max HP is computed from your classes and CON and *already* assumes the average, so the number simply moves on its own. A **rolled** result has nowhere else to live, so the difference between your roll and the average is folded into the **Max HP override** box — and once that box is in use it shadows the formula entirely, so every later level-up adds its hit points there too (including whatever per-level bonuses the formula itself would have applied, e.g. a Hill Dwarf's Dwarven Toughness). Current HP rises with the maximum. Multiclassing prerequisites (PHB p163) are **not** checked for you; the dialog says so.
...
- **A step-by-step character creator — done**, along with multi-character tabs and a Level Up dialog; see [Characters](#characters-tabs-creation-levelling-up). Rules-as-written defaults with house-rule toggles, and an allowed-books toggle list, are still planned. Known limits of what shipped: the wizard doesn't hand out class/background starting equipment packs (5e.tools' `startingEquipment` needs its own resolver) or pick skill/tool proficiencies for you, and Level Up doesn't check multiclassing ability prerequisites (PHB p163) or walk you through an Ability Score Improvement — it lists the ASI as a gained feature and leaves the scores to you.
