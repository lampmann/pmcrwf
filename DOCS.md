...
- Coverage today (556 entries): a handful of hand-written entries proving the schema (`effects/db/handwritten.js`), a full pass over the 2014 feats (`effects/db/feats.js`), and a pass over class/subclass features (`effects/db/classes-batch-*.js`) covering **every class** — artificer, barbarian, bard, cleric, fighter, monk, paladin, ranger, rogue, sorcerer, warlock, wizard, and the sidekick classes. Cleric, druid, fighter and monk still have unconverted features within them; races aren't started. All of it is generated by an LLM conversion pass over your `data/` folder and merged only after passing `effects/tools/validate-db.js` — see `effects/tools/conversion-guide.md` for the schema/process used. Nothing in it talks to your character or your `data/` folder without you explicitly running it.
- **What "converted" means for a class.** A lot of class features are *recognized but not applied*, and that's the honest state rather than a gap in the conversion: the engine's target list (initiative, saves, skills, passive Perception, max HP, spell DC/attack, proficiency bonus, ability scores) has no room for AC, speed, resistances, temporary hit points, or action economy, so features resting on those are serialized as `⚠ not automated` with the reason on hover. Four limits account for most of it: **level-banded dice** (Sneak Attack, Martial Arts, Psychic Blades — `when` has only `minLevel`, and `adddice` takes a literal string, so a die count that scales with level can't be written); **resistances and temp HP**, which have no model; **speed and AC**, which aren't effects targets; and **optional-feature files** (Fighting Styles, Metamagic, Eldritch Invocations), which the sheet doesn't import yet. Features that only change *another creature's* numbers (Cutting Words, Unsettling Words) are omitted outright rather than marked, since nothing on your sheet would move either way.
- Classes not yet converted still get their **limited-use pip trackers** from `effects/db/uses-classes.js` (and races from `uses-races.js`) — a `uses`-only stopgap produced by the deterministic scanner `effects/tools/generate-uses.js`, covering the finite-use features it can recognize but none of their other mechanics. When a class gets a full conversion pass, its keys move out of that file and into the batch: `registerEffects()` assigns whole entries rather than merging fields, so a key defined in both places would erase one side or the other rather than combine them. `generate-uses.js` skips already-converted keys automatically, and `validate-db.js` fails on any duplicate that slips through.
...
- **[Feature effects](#feature-effects-automatic-mechanics)** cover all 2014 feats (`effects/db/feats.js`) and all twelve classes plus the sidekicks (`effects/db/classes-batch-*.js`), against only those targets that already exist on the sheet (initiative, saves, skills, passive perception, max HP, spell DC/attack, proficiency bonus, ability scores). Effects that need a weapons/attacks table (Sharpshooter, GWM, Improved Divine Smite, Hexblade's Curse, Sacred Weapon, and similar) are recognized and serialized but stay disabled until that module exists. **Still to convert:** every race, plus the remaining unconverted features inside cleric, druid, fighter and monk — they fall back to `uses`-only pip trackers in the meantime.
- **Engine gaps that block whole categories of class feature** (each would unlock a batch of currently-`⚠ not automated` entries):
  ...
  - No `ac` or `speed` target. Both are painted by `paintEffectAudit()` already but nothing in `derived.js` reads `effFlat` for them, so writing to either would silently do nothing.
  - No resistances / immunities / temporary-hit-point model, which is most of what high-level subclass capstones do.
  - No "only when not proficient" predicate (Jack of All Trades) and no per-die floor (Reliable Talent, Silver Tongue, Ear for Deceit).
