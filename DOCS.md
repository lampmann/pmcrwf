...
- Coverage today: a handful of hand-written entries proving the schema (`effects/db/handwritten.js`), a full pass over the 2014 feats (`effects/db/feats.js`), and full **limited-use trackers** for races and classes/subclasses (`effects/db/uses-races.js`, `effects/db/uses-classes.js` — generated deterministically by `effects/tools/generate-uses.js` scanning `data/class/*.json`/`data/races.json` for explicit finite-use + recharge phrasing, so features whose use-count itself scales by level like Action Surge or Channel Divinity are deliberately skipped rather than given a wrong flat number).
  Beyond uses-tracking, **every 2014 race/subrace trait** has also had a full conversion pass (not just uses) — `effects/db/races-batch-01.js` through `races-batch-08.js`, extracted by `effects/tools/extract-race-features.js` (mirrors the class extractor, same text-flattening as the live app) and converted per `effects/tools/conversion-guide.md`. Most traits are flavor/out-of-scope (darkvision, resistances, languages, size, speed, innate spellcasting without a numeric bonus) and are omitted; a good number are recognized-but-not-yet-automatable and marked `unsupported` (mostly condition-specific advantage/resistance, since the engine only has per-ability save targets, not per-condition); the rest — around 55 entries — carry real automated effects (fixed/choice-driven skill and save proficiencies, a couple of advantage grants, and HP-per-level traits like Dwarven Toughness/Stensia's Tough). **All of the above — feats, hand-written entries, both uses-trackers, and the race batches — are wired into `character-sheet.html` and live.**
  A parallel pass over **class/subclass numeric effects** (beyond their uses-trackers) is in progress — `effects/db/classes-batch-01.js` through `classes-batch-12.js` — but **not yet wired into `character-sheet.html`**; `node effects/tools/validate-db.js` also currently reports 33 pre-existing duplicate-key conflicts between those batch files and `uses-classes.js` (same feature, key declared twice) that need resolving before they're wired in.
...
- **[Feature effects](#feature-effects-automatic-mechanics)** now cover all 2014 feats (`effects/db/feats.js`), **all 2014 races/subraces** (`effects/db/races-batch-01.js`–`08.js`, full numeric-effects pass, not just uses), limited-use trackers for every race and class/subclass feature with an explicit finite-use count (`effects/db/uses-races.js`, `effects/db/uses-classes.js`) — all wired into `character-sheet.html` and live. A growing (but not yet wired-in) set of full class/subclass numeric effects also exists (`effects/db/classes-batch-*.js` — see [Feature effects](#feature-effects-automatic-mechanics) for what's converted vs. loaded). Effects only target what already exists on the sheet (initiative, saves, skills, passive perception, max HP, spell DC/attack, proficiency bonus, ability scores); anything needing a weapons/attacks table (Sharpshooter, GWM, and similar) is recognized and serialized as reserved rather than applied. Remaining work: resolve the duplicate-key conflicts between the class batches and `uses-classes.js`, wire the class batches into `character-sheet.html`, and finish converting the rest of the classes/subclasses.
- **Missing vs. big-name sheets (D&D Beyond, Roll20, Fight Club 5e), still worth doing:**
  - Weapons/attacks table with to-hit and damage roll buttons — **mostly done** (the Attacks module, plus Offensive Routines' damage-by-AC-range tables with auto-doubled crit dice). Still to wire: the effects engine's reserved `attack-hit`/`damage-bonus` targets (Sharpshooter/GWM), and melee/ranged/thrown niceties.
  ...
  - Short/long rest buttons that auto-restore HP, hit dice, and spell slots (today's Short/Long Rest buttons only reset [feature-effect uses trackers](#feature-effects-automatic-mechanics), not HP/hit dice/slots themselves).
