...
function hrRenderRulings() {
  ...
  if (!all.length) {
    return `<div class="hint">No rulings loaded. Load a preset from the bar above &mdash; a ruleset carries its
      rulings along with its bans and settings.</div>`;
  }
  ...
  const relevantHtml = relevant.length ? `<div class="ruling-group">
    ...
    <div class="hint">Matched against your classes, subclasses, spells, feats, items and race &mdash; these also appear in their own sections below.</div>
  ...
  return `<div class="hint">${all.length} rulings &mdash; <b>${autoCount}</b> the sheet applies for you,
      <b>${all.length - autoCount}</b> yours to apply at the table. The second number is the larger one on purpose:
      most of what a ruleset decides is adjudication, not arithmetic.</div>
